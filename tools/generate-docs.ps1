...
$customEntryDocs = @{
  ...
  "stfr" = [ordered]@{
    ...
    Description = @(
      ...
      "The most important operator-facing interaction is that the STFR does not treat temperature, pressure, and cooling as separate subsystems. Fuel injection and fusion output raise core temperature, shield permeability moves part of the reacting contents into the case, case temperature and case pressure then rise together, and the hotter case starts boiling coolant into steam while also feeding back into later heat-transfer and spin-drag calculations.",
      "Theoretical stable values in the STFR are therefore not a single hard-coded safe number. They are an equilibrium band where core heating, core-to-case heat transfer, coolant boiloff, case venting, and shield stress all push against each other strongly enough that temperature and pressure stop drifting and instead oscillate around nearly the same values from tick to tick."
    )
    Values = @(
    ...
    Calculations = @(
      ...
      "Value interaction chain: more terf_data_J fuel injection means more core contents to react; more reaction output raises core temperature; hotter core contents and higher shield permeability move more stress and material into the case; a hotter, denser case then raises both cooling demand and containment risk.",
      ...
      "Permeability and venting form the pressure-control loop. terf_data_G bleeds core contents into the case, increasing case density, while terf_data_I vents case contents back out. Core pressure and case pressure only stay flat when those two flows are close to balancing each other.",
      "Heat transfer from core to case scales with core spin, case pressure, and capped core temperature. If the core is hotter than the case, higher pressure makes the reactor dump heat into the case faster, which in turn raises case temperature and activates stronger coolant boiling.",
      "Theoretical stable values are equilibrium conditions, not fixed constants: core_temp_change from fusion plus power lasers is approximately canceled by core heat loss, and case_temp_change from incoming heat is approximately canceled by cooling_rate and vent-driven mass removal. When that happens, temperature and pressure reinforce a feedback loop that holds both near the same operating band instead of running away.",
